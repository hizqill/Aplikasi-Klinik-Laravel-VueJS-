    ...
    <div class="min-h-screen bg-slate-100 pt-8">
        <div class="md:mx-24 mt-8 bg-white  rounded-lg shadow-xl">
            ...
            <div v-if="data_pasien" class="mt-6 mx-8 flex gap-2">
                <div  @click="HandleDataPasien" class="px-4 py-2 w-36 text-center  bg-slate-700 hover:bg-slate-500 rounded-lg text-white cursor-pointer">
                    Data Pasien
                </div>
                <div  @click="HandleTambahPasien" class="px-4 py-2 w-36 text-center border  border-slate-500 text-slate-600 hover:bg-slate-600 rounded-lg hover:text-white cursor-pointer">
                    Tambah Pasien
                </div>
            </div>
            <div v-if="tambah_pasien" class="mt-6 mx-8 flex gap-2">
                <div @click="HandleDataPasien" class="px-4 py-2 w-36 text-center border  border-slate-500 text-slate-600 hover:bg-slate-600 rounded-lg hover:text-white cursor-pointer">
                    Data Pasien
                </div>
                <div  @click="HandleTambahPasien" class="px-4 py-2 w-36 text-center bg-slate-700 hover:bg-slate-500 rounded-lg text-white cursor-pointer">
                    Tambah Pasien
                </div>
            </div>
                        ...
                        <div class="overflow-hidden ">
                            <table class="min-w-full divide-y divide-gray-200 table-fixed ">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200 ">
                                    ...
                                    <tr class="hover:bg-gray-100 " v-for="pas in pasien" :key="pas.id">
                                        <td class="py-4 px-6 text-sm font-medium text-gray-900 whitespace-nowrap ">{{ pas.nama }}</td>
                                        <td class="py-4 px-6 text-sm font-medium text-gray-500 whitespace-nowrap ">{{ pas.alamat }}</td>
                                        <td class="py-4 px-6 text-sm font-medium text-gray-900 whitespace-nowrap ">{{ pas.keluhan }}</td>
                                        <td class="py-4 px-6 text-sm font-medium text-gray-900 whitespace-nowrap ">{{ pas.tanggal_datang }}</td>
                                        <td class="py-4 px-6 text-sm font-medium text-gray-900 whitespace-nowrap ">{{ pas.dokter_nama }}</td>
                                        <td class="py-4 px-6 text-sm font-medium whitespace-nowrap border flex justify-center space-x-2">
                                            <button @click="HandleEditProduk(pas.id, pas.nama, pas.alamat, pas.tanggal_datang, pas.keluhan, pas.dokter_id)" type="button" class="bg-amber-400 hover:bg-amber-600  p-1.5 rounded-sm text-black   hover:text-white">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
                                                    <path d="M15.502 1.94a.5.5 0 0 1 0 .706L14.459 3.69l-2-2L13.502.646a.5.5 0 0 1 .707 0l1.293 1.293zm-1.75 2.456-2-2L4.939 9.21a.5.5 0 0 0-.121.196l-.805 2.414a.25.25 0 0 0 .316.316l2.414-.805a.5.5 0 0 0 .196-.12l6.813-6.814z" />
                                                    <path fill-rule="evenodd" d="M1 13.5A1.5 1.5 0 0 0 2.5 15h11a1.5 1.5 0 0 0 1.5-1.5v-6a.5.5 0 0 0-1 0v6a.5.5 0 0 1-.5.5h-11a.5.5 0 0 1-.5-.5v-11a.5.5 0 0 1 .5-.5H9a.5.5 0 0 0 0-1H2.5A1.5 1.5 0 0 0 1 2.5v11z" />
                                                </svg>
                                            </button>
                                            <button @click="HandleButtonHapusPasien(pas.id, pas.nama)" type="button" class="bg-red-500 hover:bg-red-700 p-1.5 rounded-sm text-white hover:underline">
                ...
                <div class="px-8 py-6 ">
                    <div >
                        <div class="relative z-0 mb-6 w-full group">
                            <input type="text" v-model="form.nama" class="block py-2.5 px-0 w-full text-sm
                        ...
                        <div class="relative z-0 mb-6 w-full group">
                            <input type="text" v-model="form.alamat" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
                            <label class="absolute text-sm text-gray-500 duration-300 transform -translate-y-6
                            scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500
                            peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75
                        ...
                        <div class="relative z-0 mb-6 w-full group">
                            <input type="text" v-model="form.keluhan" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
                            <label class="absolute text-sm text-gray-500 duration-300 transform -translate-y-6
                            scale-75 top-3 -z-10 origin-[0] peer-focus:left-0 peer-focus:text-blue-600 peer-focus:dark:text-blue-500
                            peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75
                        ...
                        <div class="relative z-0 mb-6 w-full group">
                            ...
                            <select v-model="form.id_dokter"  class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                                <option value="">-- Pilih Dokter --</option>

                                <option v-for="item in dokter" :key="item.id" :value="item.id" >{{ item.nama }}</option>
                        ...
                        <div class="relative z-0 mb-6 w-full group">
                            ...
                            <div class="relative">
                                ...
                                <input  type="datetime-local" v-model="form.tgl_masuk"
    ...
    methods: {
        ...
        HandleButtonSubmitTambahPasien(){
            ...
            let formData = {
                ...
                tanggal_datang: this.form.tgl_masuk,
                keluhan: this.form.keluhan,
                dokter_id: this.form.id_dokter,
            };
            axios({
                    method: "post",
                    url: "/api/pasien-create",
                    data: formData,
                })
                .then((response) => {
                    this.Get_Pasien()
        ...
        HandleButtonSubmitEditPasien(){
            ...
            axios({
                    method: "post",
                    url: "/api/pasien-edit/" + this.form.id,
                    data: formData,
                })
                .then((response) => {
                    this.Get_Pasien()
        ...
        HandleEditProduk(id, nama, alamat, tgl_masuk, keluhan, id_dokter){
            this.data_pasien = false
            this.tambah_pasien = false
            ...
            this.form.tgl_masuk = tgl_masuk
            this.form.keluhan = keluhan
            this.form.id_dokter = id_dokter
        },


         HandleButtonHapusPasien(id, nama){
            Swal.fire({
